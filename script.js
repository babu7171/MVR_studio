...
(function () {
  ...
  // ── Render Bookings inside Admin Panel ──
  function renderAdminBookings() {
    const listDiv = document.getElementById('adminBookingsList');
    const clearBtn = document.getElementById('btnClearAllBookings');
    if (!listDiv) return;

    const enquiries = getEnquiries();
    if (enquiries.length === 0) {
      listDiv.innerHTML = `
        <div style="text-align: center; padding: 30px 20px; background: rgba(255,255,255,0.01); border: 1px dashed rgba(255,255,255,0.08); border-radius: var(--r2); color: var(--g3); font-size: 0.9rem;">
    ...
    listDiv.innerHTML = enquiries.map(item => {
      ...
      // 2. Booking Date & Month Formatting (e.g. "02/06/2026, 17:08:29" -> "02/06/2026")
      let bookingDateFormatted = item.timestamp || '';
      if (item.timestamp) {
        try {
          const datePart = item.timestamp.split(',')[0];
          bookingDateFormatted = datePart;
        } catch (e) {}
      }
        ...
        <div class="admin-booking-card" id="adminCard-${item.id}">
          <div class="admin-booking-card-top">
            ...
            <div class="admin-booking-actions">
              ...
              <button onclick="window.deleteAdminBooking(${item.id})" class="btn-ghost" style="padding: 4px 10px; font-size: 0.76rem; border-radius: var(--r1); border-color: rgba(255,82,82,0.3); color: #ff5252; cursor: pointer; background: transparent;">
                Delete
              </button>
            </div>
          </div>

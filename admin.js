(function() {
  ...
  // LocalStorage keys
  const ENQUIRIES_KEY = 'mvr_enquiries';
  const GALLERY_KEY = 'mvr_gallery';
  const STATS_KEYS = {
  ...
  function renderBookings() {
    ...
    container.innerHTML = enquiries.map(item => {
      // 1. Event Date & Month Formatting (e.g. "2026-06-16" -> "16 Jun 2026")
        ...
        <div class="admin-booking-card" id="adminCard-${item.id}">
          <div class="admin-booking-card-top">
            ...
            <div class="admin-booking-actions">
              ...
              <button onclick="deleteBooking(${item.id})" class="btn-ghost" style="padding: 4px 10px; font-size: 0.76rem; border-radius: var(--r1); border-color: rgba(255,82,82,0.3); color: #ff5252; cursor: pointer; background: transparent;">
                Delete
              </button>
            </div>
          </div>
  ...
  // Make deleteBooking global for inline onclick
  ...
  function initStatsEditor() {
    ...
    // Check local storage values, fall back to HTML defaults (500, 8, 100, 50)
    inputProjects.value = localStorage.getItem(STATS_KEYS.projects) || '500';
    inputYears.value = localStorage.getItem(STATS_KEYS.years) || '8';
    inputSatisfaction.value = localStorage.getItem(STATS_KEYS.satisfaction) || '100';
    inputReviews.value = localStorage.getItem(STATS_KEYS.reviews) || '50';

    statForm.addEventListener('submit', e => {
      e.preventDefault();
      try {
        localStorage.setItem(STATS_KEYS.projects, inputProjects.value);
        localStorage.setItem(STATS_KEYS.years, inputYears.value);
        localStorage.setItem(STATS_KEYS.satisfaction, inputSatisfaction.value);
        localStorage.setItem(STATS_KEYS.reviews, inputReviews.value);
        alert('Website counters saved successfully!');
      } catch (err) {
        alert('Error saving counter stats.');
      }
    });
  }
  ...
  function initGalleryManager() {
    ...
    function saveGallery(items) {
      try {
        ...
        alert('Storage space is full! Try removing some old uploaded photos or videos.');
      }
    }
    ...
    function renderGalleryPreview() {
      ...
      if (items.length === 0) {
        previewContainer.innerHTML = `
          <div style="grid-column: 1 / -1; text-align: center; padding: 30px; background: rgba(255,255,255,0.01); border: 1px dashed rgba(255,255,255,0.08); border-radius: var(--r2); color: var(--g3); font-size: 0.85rem;">
      ...
      previewContainer.innerHTML = items.map((item, idx) => {
        ...
        return `
          <div class="admin-gal-card">
            ${mediaHTML}
            <button onclick="deleteGalleryItem(${idx})" class="admin-gal-del" title="Remove Item">✕</button>
